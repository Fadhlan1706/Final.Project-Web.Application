...
_Conventional Commits_ adalah perjanjian sederhana tentang cara menulis pesan komit.
...
2. **feat:** komit _tipe_ `feat` memperkenalkan suatu fitur (feature) baru dalam kode Anda (ini berkaitan dengan [`MINOR`](http://semver.org/#summary) di Semantic Versioning).
3. **BREAKING CHANGE:** komit yang berisi footer `BREAKING CHANGE:`, atau tambahkan `!` setelah type/scope, memperkenalkan perubahan yang merusak API (ini berkaitan dengan [`MAJOR`](http://semver.org/#summary) di Semantic Versioning). BREAKING CHANGE dapat diterapkan pada semua _tipe_ komit.
4. _types_ selain `fix:` dan `feat:` diperbolehkan, sebagai contoh [@commitlint/config-conventional](https://github.com/conventional-changelog/commitlint/tree/master/%40commitlint/config-conventional) (berdasarkan pada [the Angular convention](https://github.com/angular/angular/blob/22b96b9/CONTRIBUTING.md#-commit-message-guidelines)) disarankan `build:`, `chore:`,
   `ci:`, `docs:`, `style:`, `refactor:`, `perf:`, `test:`, dan lainnya.
5. _footers_ daripada `BREAKING CHANGE: <description>` dapat disediakan dan mengikuti konvensi yang mirip dengan
...
| Commit Type | Judul                    | Deskripsi                                                                                                                | Emoji |
| ----------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------ | :---: |
| `feat`      | Features                 | Menambahkan fitur baru                                                                                                   |  ✨   |
| `fix`       | Bug Fixes                | Memperbaiki bug atau kesalahan pada sistem                                                                               |  🐛   |
| `docs`      | Documentation            | Perubahan yang hanya berkaitan dengan dokumentasi                                                                        |  📚   |
| `style`     | Styles                   | Perubahan yang tidak memengaruhi makna atau perilaku kode (white-space, formatting, missing semi-colons, dan sebagainya) |  💎   |
| `refactor`  | Code Refactoring         | Perubahan kode yang tidak memperbaiki bug maupun menambahkan fitur baru                                                  |  📦   |
| `perf`      | Performance Improvements | Perubahan kode yang bertujuan meningkatkan performa                                                                      |  🚀   |
| `test`      | Tests                    | Menambahkan test yang belum ada atau memperbaiki test yang sudah ada                                                     |  🚨   |
| `build`     | Builds                   | Perubahan yang memengaruhi build system atau external dependencies (contoh scope: gulp, broccoli, npm)                   |  🛠   |
| `ci`        | Continuous Integrations  | Perubahan pada file konfigurasi dan script CI (contoh scope: Travis, Circle, BrowserStack, SauceLabs)                    |  ⚙️   |
| `chore`     | Chores                   | Perubahan lain yang tidak memodifikasi file source code (src) maupun file test                                           |  ♻️   |
| `revert`    | Reverts                  | Membatalkan (revert) commit sebelumnya                                                                                   |  🗑   |
...
Kami menyarankan Anda melanjutkan seolah-olah Anda sudah merilis produk. Biasanya _seseorang_, bahkan jika itu sesama pengembang perangkat lunak Anda, menggunakan perangkat lunak Anda. Mereka ingin tahu apa yang diperbaiki, apa yang rusak, dll.
...
#### Saat Anda menggunakan jenis _bukan_ spesifikasi, mis. `feet` bukannya `feat`
